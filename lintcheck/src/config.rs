...
fn get_clap_config() -> ArgMatches {
    Command::new("lintcheck")
        .about("run substrace on a set of crates and check output")
        .args([
            Arg::new("only")
            ...
            Arg::new("fix")
                .long("fix")
                .help("Runs cargo substrace --fix and checks if all suggestions apply"),
            Arg::new("filter")
                .long("filter")
                .action(ArgAction::Append)
                .value_name("substrace_lint_name")
            ...
            Arg::new("recursive")
                .long("--recursive")
                .help("Run substrace on the dependencies of crates specified in crates-toml")
...
pub(crate) struct LintcheckConfig {
    ...
    pub markdown: bool,
    /// Run substrace on the dependencies of crates
    pub recursive: bool,
}
        ...
        let lint_filter: Vec<String> = clap_config
            ...
            .map(|iter| {
                iter.map(|lint_name| {
                    ...
                    if !filter.starts_with("substrace::") {
                        filter.insert_str(0, "substrace::");
                    }
                    filter
                })
                .collect()
