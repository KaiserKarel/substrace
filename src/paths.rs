...
pub fn is_insecure_hash_function<'hir>(
    cx: &LateContext<'hir>,
    generics: &hir::GenericArgs,
) -> bool {
        ...
        if let hir::GenericArg::Type(ty) = typ {
            if let hir::TyKind::Path(hir::QPath::Resolved(_, path)) = &ty.kind {
                if let hir::def::Res::Def(hir::def::DefKind::Struct, id) = path.res {
                    return match_def_path(cx, id, &TWOX64CONCTAT)
...
pub fn modifies_storage_double_map(cx: &LateContext<'_>, fn_def_id: DefId) -> bool {
    match_def_path(cx, fn_def_id, &ITERABLE_STORAGE_DOUBLE_MAP_SWAP)
        ...
        || match_def_path(
            cx,
            fn_def_id,
            &ITERABLE_STORAGE_DOUBLE_MAP_TRY_MUTATE_EXISTS,
        )
...
pub const STORAGE_MAP: [&str; 3] = ["frame_support", "storage", "StorageMap"];
pub const STORAGE_DOUBLE_MAP: [&str; 3] = ["frame_support", "storage", "StorageMap"];
pub const STORAGE_N_MAP: [&str; 3] = ["frame_support", "storage", "StorageNMap"];

pub const TWOX64CONCTAT: [&str; 2] = ["frame_support", "Twox64Concat"];
pub const TWOX128: [&str; 2] = ["frame_support", "Twox128"];
pub const TWOX256: [&str; 2] = ["frame_support", "Twox256"];

pub const IDENTITY: [&str; 2] = ["frame_support", "Identity"];
