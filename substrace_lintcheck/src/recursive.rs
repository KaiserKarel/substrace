...
//! substrace on the crate to the server

use crate::SubstraceWarning;
...
fn process_stream(
    stream: TcpStream,
    sender: &Sender<SubstraceWarning>,
    ...
    let messages = stderr
        ...
        .filter_map(|diag| SubstraceWarning::new(diag, &driver_info.package_name, &driver_info.version));
...
pub(crate) struct LintcheckServer {
    ...
    receiver: Receiver<SubstraceWarning>,
    sender: Arc<Sender<SubstraceWarning>>,
}
...
impl LintcheckServer {
    pub fn spawn(options: RecursiveOptions) -> Self {
        ...
        let (sender, receiver) = crossbeam_channel::unbounded::<SubstraceWarning>();
    ...
    pub fn warnings(self) -> impl Iterator<Item = SubstraceWarning> {
