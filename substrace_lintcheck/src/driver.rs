...
/// 3. Otherwise sends the stderr of running `substrace-driver` to the server
fn run_substrace(addr: &str) -> Option<i32> {
    ...
    // Remove --cap-lints allow so that substrace runs and lints are emitted
    ...
    let output = Command::new(env::var("SUBSTRACE_DRIVER").expect("missing env SUBSTRACE_DRIVER"))
        .args(args)
        .stdout(Stdio::inherit())
        .output()
        .expect("failed to run substrace-driver");
...
pub fn drive(addr: &str) {
    process::exit(run_substrace(addr).unwrap_or_else(|| {
        Command::new("rustc")
