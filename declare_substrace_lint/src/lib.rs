...
impl Parse for SubstraceLint {
    fn parse(input: ParseStream<'_>) -> Result<Self> {
        ...
        for attr in &attrs {
            ...
            } else if let Some(lit) = parse_attr(["substrace", "version"], attr) {
                if let Some(duplicate) = version.replace(lit) {
                    return Err(Error::new_spanned(duplicate, "duplicate substrace::version"));
                }
            } else {
...
///
/// 1. The documentation, which is used for the website and `cargo substrace --explain`
...
#[proc_macro]
pub fn declare_substrace_lint(input: TokenStream) -> TokenStream {
    let SubstraceLint {
        attrs,
        explanation,
        name,
        category,
        description,
    } = parse_macro_input!(input as SubstraceLint);
