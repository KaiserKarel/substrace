...
pub fn create(
    ...
    if category == Some("cargo") && ty.is_none() {
        // `cargo` is a special category, these lints should always be in `substrace_lints/src/cargo`
        ty = Some("cargo");
    }
    ...
    if lint.ty.is_none() {
        add_lint(&lint, msrv).context("Unable to add lint to substrace_lints/src/lib.rs")?;
    }
...
fn create_lint(lint: &LintData<'_>, enable_msrv: bool) -> io::Result<()> {
    if let Some(ty) = lint.ty {
        ...
        let lint_path = format!("substrace_lints/src/{}.rs", lint.name);
...
fn add_lint(lint: &LintData<'_>, enable_msrv: bool) -> io::Result<()> {
    let path = "substrace_lints/src/lib.rs";
    ...
    result.push_str(&if enable_msrv {
        format!(
            indoc! {"
                ...
                // TODO: Update msrv config comment in `substrace_lints/src/utils/conf.rs`
            "},
            pass_type = pass_type,
...
fn create_lint_for_ty(lint: &LintData<'_>, enable_msrv: bool, ty: &str) -> io::Result<()> {
    ...
    let ty_dir = lint.project_root.join(format!("substrace_lints/src/{}", ty));
    assert!(
    ...
    if enable_msrv {
        let _ = writedoc!(
            lint_file_contents,
            r#"
                use substrace_utils::{{meets_msrv, msrvs}};
                ...
                pub(super) fn check(cx: &{context_import}, msrv: Option<RustcVersion>) {{
                    if !meets_msrv(msrv, todo!("Add a new entry in `substrace_utils/src/msrvs`")) {{
                        return;
                    }}
                    todo!();
                }}
    ...
    println!("Generated lint file: `substrace_lints/src/{}/{}.rs`", ty, lint.name);
    println!(
        "Be sure to add a call to `{}::check` in `substrace_lints/src/{}/mod.rs`!",
        lint.name, ty
    );
...
fn setup_mod_file(path: &Path, lint: &LintData<'_>) -> io::Result<&'static str> {
    ...
    // Find both the last lint declaration (declare_substrace_lint!) and the lint pass impl
