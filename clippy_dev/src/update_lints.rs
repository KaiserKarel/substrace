...
fn generate_lint_files(
    ...
    replace_region_in_file(
        update_mode,
        Path::new("substrace_lints/src/lib.rs"),
    ...
    process_file(
        "substrace_lints/src/lib.register_lints.rs",
        update_mode,
        &gen_register_lint_list(internal_lints.iter(), usable_lints.iter()),
    );
    process_file(
        "substrace_lints/src/lib.deprecated.rs",
        update_mode,
    ...
    process_file("substrace_lints/src/lib.register_all.rs", update_mode, &content);
    ...
    for (lint_group, lints) in Lint::by_lint_group(usable_lints.into_iter().chain(internal_lints)) {
        ...
        process_file(
            &format!("substrace_lints/src/lib.register_{}.rs", lint_group),
            update_mode,
            &content,
        );
    }
...
pub fn rename(old_name: &str, new_name: &str, uplift: bool) {
    ...
    write_file(
        Path::new("substrace_lints/src/renamed_lints.rs"),
    ...
    if uplift {
        ...
        println!(
            "`{}` has be uplifted. All the code inside `substrace_lints` related to it needs to be removed manually.",
            old_name
        );
    } else if found_new_name {
        write_file(Path::new("tests/ui/rename.rs"), &gen_renamed_lints_test(&renamed_lints));
        println!(
            "`{}` is already defined. The old linting code inside `substrace_lints` needs to be updated/removed manually.",
            new_name
        );
    } else {
        ...
        let replacements = if lint.module == old_name
            && try_rename_file(
                Path::new(&format!("substrace_lints/src/{}.rs", old_name)),
                Path::new(&format!("substrace_lints/src/{}.rs", new_name)),
            ) {
            ...
            && try_rename_file(
                Path::new(&format!("substrace_lints/src/{}/{}.rs", lint.module, old_name)),
                Path::new(&format!("substrace_lints/src/{}/{}.rs", lint.module, new_name)),
            )
        {
...
pub fn deprecate(name: &str, reason: Option<&String>) {
    fn finish(
        ...
        if reason == DEFAULT_DEPRECATION_REASON {
            println!("note: the deprecation reason must be updated in `substrace_lints/src/deprecated_lints.rs`");
        }
    ...
    let mod_path = {
        let mut mod_path = PathBuf::from(format!("substrace_lints/src/{}", lint.module));
        if mod_path.is_dir() {
    ...
    let deprecated_lints_path = &*clippy_project_root().join("substrace_lints/src/deprecated_lints.rs");
    ...
    if path.exists() {
        if let Some(lint) = lints.iter().find(|l| l.name == name) {
            if lint.module == name {
                ...
                // Remove lint declaration (declare_substrace_lint!)
...
fn clippy_lints_src_files() -> impl Iterator<Item = (PathBuf, DirEntry)> {
    let root_path = clippy_project_root().join("substrace_lints/src");
...
/// Parse a source file looking for `declare_substrace_lint` macro invocations.
fn parse_contents(contents: &str, module: &str, lints: &mut Vec<Lint>) {
    ...
    while let Some(LintDeclSearchResult { range, .. }) = iter.find(
        |LintDeclSearchResult {
             token_kind, content, ..
         }| token_kind == &TokenKind::Ident && *content == "declare_substrace_lint",
    ) {
